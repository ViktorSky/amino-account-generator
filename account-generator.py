...
#    Path(parameters["save-path"]).mkdir(exist_ok=True)
...
class AccountGenerator:
    ...
    def captcha_solver(
        ...
        #with suppress(Exception):
#            response = session.get(url="https://cynical-is-gay.herokuapp.com/captcha/solver/V1?data=captcha")
#            return response.json()['captcha']
            
        with suppress(Exception):
            response = session.post(url="http://192.46.210.24:5000/captcha", data={"data": captcha})
            print(response.text)
            return response.json()['dick']
    ...
    while True:
        try:
            ...
            with suppress(IndexError):
                print("~", repr(accounts[-1]['email']), "saved!")
            ...
            print("[ERROR]", RED(Error.args[0]["api:message"]))
            sleep(4)
